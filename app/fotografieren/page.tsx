...
export default function FotografierenPage() {
  ...
  const fetchRecentProducts = useCallback(async () => {
    try {
      ...
      console.warn('[Fotografieren] Failed to fetch recent products:', err);
    }
  }, []);
    ...
    const createProduct = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error('[Fotografieren] POST /api/products failed:', res.status, errorBody);
          throw new Error(
            res.status === 401
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-xl font-semibold">Artikel Fotografieren</h1>
      </div>
